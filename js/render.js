(function(){
  ...
  mount('positions-list',d.positions.map((x,i)=>`<li class="reveal flex gap-3"><span class="mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-gold-400/15 text-xs font-bold text-gold-500">${i+1}</span><span>${x}</span></li>`).join(''));
  ...
  if(isProjectsPage){
    ...
    mount('projects-grid',`<div class="projects-carousel reveal" tabindex="0" role="region" aria-label="Key projects and initiatives — scroll or drag to browse"><div class="projects-carousel-track">${cards}</div></div>`);
    ...
    mount('projects-grid',`<div class="initiatives-marquee reveal" tabindex="0" role="region" aria-label="Selected initiatives — scroll or drag to browse"><div class="initiatives-track">${cards}</div></div>`);
  ...
  const renderPublications=(publications)=>{
    ...
    const featuredHtml=`<a href="${featured.url}" target="_blank" rel="noopener noreferrer" class="pub-featured reveal group">
    ...
    const gridHtml=rest.map((x)=>`<a href="${x.url}" target="_blank" rel="noopener noreferrer" class="pub-grid-item reveal group block">
  ...
  mount('recognition-grid',d.recognition.map(x=>`<article class="reveal border-l-2 border-gold-400 pl-5"><h3 class="text-2xl font-semibold text-navy-900 dark:text-white">${x.title}</h3><p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">${x.text}</p></article>`).join(''));
  mount('extras-list',d.extras.map(x=>`<li class="reveal rounded-xl bg-white p-4 text-sm shadow-sm dark:bg-slate-900">${x}</li>`).join(''));
})();